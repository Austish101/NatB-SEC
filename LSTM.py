...
class SplitLSTM:
    ...
    def create_model(self, input_shape, output_shape, output_activation, hidden_activation, learning_rate, loss):
        model = Sequential()
        model.add(LSTM(self.nodes_in_layer, input_shape=(input_shape[0], input_shape[1]), activation=hidden_activation))
    ...
    # predict using real data, data must be shaped correctly (3 packets at a time)
    def predict_real(self, inputs, error_threshold=0.8, time_threshold=500):
    ...
    def predict_test(self, test_input, test_output, score_or_stats, error_threshold=0.8, time_threshold=500):
        ...
        if score_or_stats == "stats":
            ...
            for i in range(0, test_input.shape[0]):
                i_input = [test_input[i]]
                error_predict = self.error_model.predict(i_input)
                time_predict = self.time_model.predict(i_input)

                current_time = test_input[i][0]
                ...
                for n in range(0, error_predict.shape[0]):
                    if error_predict[n] > error_threshold and time_to_error < time_threshold:
                        ...
                        expected_timestamp = self.unstd_time(test_time)
                        ...
                        for e in range(0, error_predict.shape[1]):
                            if test_error[i][e] == 1:
                                if predicted_error == e:
                                    is_error_correct = True
                        stats.append([is_error_correct, is_time_correct, time_difference])
